...
for(i=0;i<heroes.length;i++){
	if(heroes[i]==='superman'){var firstHero=heroes.splice(i,1).toString();}
}
...
for(i=0;i<heroes.length;i++){
	if(heroes[i]==='flash'){var secondHero=heroes.splice(i,1).toString();}
}
// leave batman in the array but put a copy of him on a variable called thirdHero
for(i=0;i<heroes.length;i++){
	if(heroes[i]==='batman'){var thirdHero=heroes.slice(i,i+1).toString();}
}
...
function getValueOfProperty(ob,prop){
...
function coderTest(obj){
	if(obj.name==="Jeremy"){obj.lovesCode=10;}
	else if(obj.name==="Brack"){obj.lovesCode=0;}
